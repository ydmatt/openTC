...
- `v1.0.16` 已完成代码、74/74 自动化测试、正式打包和独立启动/单实例转发烟雾测试。完整目录位于 `T:\mytc-releases\MYTC-v1.0.16-win-x64`，升级 ZIP 位于 `T:\mytc-updates\MYTC-v1.0.16-win-x64.zip`。此补丁修复 v1.0.15 中 `Alt+Enter` 被误当作普通 `Enter` 的路由问题。
